...
GENERATE_JSON_RESULTS_OPTIONS = [
    ...
    'builder_base_url', 'chrome_revision', 'test_results_server', 'test_type',
    'master_name'
]
...
def GetResultsMap(observer):
  ...
  test_results_map = dict()
  tests = (
      observer.FailedTests(include_fails=True, include_flaky=True) +
...
def GetResultsMapFromXML(results_xml):
  ...
  for testcase in testcases:
    ...
    result = TestResult(
        test_name, failed=bool(failures), not_run=not_run, elapsed_time=elapsed
    )
...
def GenerateJSONResults(test_results_map, options):
  ...
  try:
    ...
    logging.error(
        'options.build_number needs to be a number: %s', options.build_number
    )
    return
...
def UploadJSONResults(generator):
  ...
  if generator:
    generator.upload_json_files([FULL_RESULTS_FILENAME, TIMES_MS_FILENAME])
...
def main():
  ...
  option_parser.add_option(
      ...
      default='',
      help=(
          'A URL where we have the archived test '
          'results. (default=%sTEST_TYPE_results/)' % BUILDER_BASE_URL
      )
  )
  option_parser.add_option(
      '',
