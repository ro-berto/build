...
def DownloadFileWithPolling(
    url, dst, poll_interval=_POLL_INTERVAL_DEFAULT, timeout=_TIMEOUT_DEFAULT
):
...
def main():
  desc = (
      'There are 2 modes of downloading a file from Google Storage.\n'
  ...
  parser.add_option('--url', help='Google Storage URL (gs://...).')
  parser.add_option(
      '--partial-name', help='Partial name of the file to download.'
  )
  ...
  parser.add_option(
      '--poll-interval',
      type=int,
      default=_POLL_INTERVAL_DEFAULT,
