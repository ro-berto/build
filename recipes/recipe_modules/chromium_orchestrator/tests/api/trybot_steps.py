...
def GenTests(api):
  ...
  yield api.test(
      ...
      api.chromium_orchestrator.override_compilator_steps(
          affected_files=['src/file1.cc', 'src/file2.cc'],
          deleted_files=['src/file2.cc'],
      ...
      api.post_process(
          ...
          'process clang code coverage data for overall test '
          'coverage.generate line number mapping from bot to Gerrit',
          ['file2.cc'],
      ),
      api.post_process(post_process.StatusSuccess),
      api.post_process(post_process.DropExpectation),
  ...
  yield api.test(
      'code_coverage_no_deleted_files_prop',
      get_try_build(remove_src_checkout_experiment=True),
      ctbc_properties(),
      api.properties(
          **{
      ...
      api.chromium_orchestrator.override_compilator_steps(
          affected_files=['src/file1.cc'], include_deleted_files=False),
      api.chromium_orchestrator.override_compilator_steps(
          is_swarming_phase=False),
      api.post_process(
          # Only generates coverage data for the with patch step.
