...
def RunSteps(api):
  ...
  api.codesearch.generate_gn_target_list(targets=api.properties.get('targets'))
...
def GenTests(api):
  ...
  yield api.test(
      'generate_gn_target_list_failed',
      api.step_data('generate gn target list', retcode=1),
  )
