...
def parse_arguments():
  ...
  parser.add_option('--binary-path', help='Path to the symupload binary.')
  parser.add_option('--build-dir', help='Path to the build output.')
  parser.add_option(
      '--platform',
...
def main():
  ...
  with open(options.api_key_file, 'r') as fn:
    api_key = fn.read()
  ...
  try:
    output = subprocess.check_output([exe_path, '-h'], stderr=subprocess.STDOUT)
    print(output.decode('utf-8'))
  except subprocess.CalledProcessError:
    pass

  artifacts = options.artifacts.split(',')
  server_urls = options.server_urls.split(',')
  ...
  for artifact in artifacts:
    for url in server_urls:
      ...
      cmd_args = build_args(platform, artifact, url, api_key)
      ...
      print('\n' +
            subprocess.list2cmdline([symupload_binary_path] + clean_args))
      ...
      try:
        ...
        # Any other non-zero ret code returned as failure
          ...
          print('Failed to upload %s to %s. Return code %s with output %s' %
                (artifact, url, ret_code, e.output))
