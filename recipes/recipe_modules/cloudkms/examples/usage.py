...
def RunSteps(api):
  ...
  api.cloudkms.sign(
      'projects/PROJECT/locations/LOCATION/keyRings/KEYRING/cryptoKeys/KEY',
      api.path['start_dir'].join('build'), api.path['start_dir'].join('bin'),
      'service_acc')
  ...
  api.cloudkms.verify(
      'projects/PROJECT/locations/LOCATION/keyRings/KEYRING/cryptoKeys/KEY',
      api.path['start_dir'].join('signed'), api.path['start_dir'].join('sign'),
      api.path['cleanup'].join('status'), 'service_acc')
