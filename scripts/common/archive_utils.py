...
def CreateArchive(build_dir, staging_dir, files_list, zip_name=None,
                  allow_missing=True):
  ...
  if zip_name:
    ...
    zip_base_name = 'chrome-%s' % chromium_utils.PlatformName()
  ...
  # Check that MakeZip() actually named the zip file the way we expect,
  # otherwise rename it to the requested name.
  if zip_name and os.path.basename(zip_file) != zip_name:
    # Make sure there are no conflicts between the zip staging directory and the
    # final zip file name. This can happen if, for instance, the requested
    # zip_name had no extension (e.g. 'archive' instead of 'archive.zip').
    if zip_name == os.path.basename(zip_dir):
      new_zip_dir = zip_dir + '.zipstaging'
      os.rename(zip_dir, new_zip_dir)
      zip_dir = new_zip_dir
    orig_zip_file = zip_file
    zip_file = os.path.join(os.path.dirname(orig_zip_file), zip_name)
    os.rename(orig_zip_file, zip_file)
