...
def Archive(run_id, gen_dir, gpu_ref_dir, sw_ref_dir):
  ...
  Args:
    ...
    sw_ref_dir: the path to the software-rendered reference image directory
  Returns:
  ...
  for filename in filenames:
    if filename.startswith('diff-'):
      if not CopyToGoogleStorage(os.path.join(gen_dir, filename),
                                 run_url + 'diff/' + filename[5:]):
        success = False
    else:
      if not CopyToGoogleStorage(os.path.join(gen_dir, filename),
                                 run_url + 'gen/' + filename):
        success = False
      # Copy the appropriate reference image.
      gpu_ref = os.path.join(gpu_ref_dir, filename)
      sw_ref = os.path.join(sw_ref_dir, filename)
      if os.path.exists(gpu_ref):
        ref = gpu_ref
      elif os.path.exists(sw_ref):
        ref = sw_ref
      else:
        print 'Cannot find reference image for generated image', filename
        success = False
        continue
      if not CopyToGoogleStorage(ref, run_url + 'ref/' + filename):
        success = False
  return success
...
def main():
  ...
  if (options.run_id is None or
      ...
      options.gpu_reference_dir is None or
      options.sw_reference_dir is None):
