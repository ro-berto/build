...
def InstallGradle(api, checkout):
  ...
  update_android_cmd.append(
      'echo y | %s update sdk --no-ui --all --filter build-tools-27.0.3,android-25,extra-android-m2repository' %
...
def RunSteps(api):
  ...
  path_prefix = api.path.pathsep.join((str(flutter_bin), str(dart_bin), str(gradle_bin)))

  if api.platform.is_win:
    path_prefix = api.path.pathsep.join((path_prefix,
                                         api.path.join('%(PROGRAMFILES)s', '7-Zip-A', 'x64')))
