...
def CalculateCodeCoverage(api, output_path, unit_test_binary, e2e_test_binary):
  """Calculates code coverage from raw coverage data generated by prior test
     runs."""
  with api.step.nest('calculate code coverage') as coverage_step:
    coverage_step.status = api.step.SUCCESS
    try:
      # Ensure tests correctly generated coverage data.
      profraw_path = output_path.join('default.profraw')
      if not api.path.exists(profraw_path):
        api.python.failing_step(
            'skip processing because no coverage data was generated', '')
      ...
      # TODO(issuetracker.google.com/155644396): Fix intermittent failures
      # in this step.
      api.python(
          'process raw coverage data',
          api.profiles.merge_results_script,
          args=[
              '--task-output-dir', output_path, '--profdata-dir', temp_dir,
...
def RunTestsLocally(api, output_path, unit_test_binary, e2e_test_binary,
                    use_coverage):
  ...
  if not use_coverage:
    api.python.succeeding_step('code coverage calculations skipped', '')
  else:
    CalculateCodeCoverage(api, output_path, unit_test_binary, e2e_test_binary)

  # TODO(issuetracker.google.com/155643967): Include E2E test coverage with
  # unit test coverage.
