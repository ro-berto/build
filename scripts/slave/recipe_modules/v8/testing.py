...
class V8Test(BaseTest):
  def __init__(self, *args, **kwargs):
    ...
    self.completion_config = DirectCompletionConfig(self.api)
  ...
  def run(self, test=None, coverage_context=NULL_COVERAGE, **kwargs):
    ...
    ] + self.completion_config.ci_progress_args()
    ...
    with self.api.context(cwd=self.api.path['checkout'], env=env):
      self.api.python(
        ...
        full_args,
        step_test_data=self.completion_config.augment_test_data(
            self.api.v8.test_api.output_json),
        **kwargs)
  ...
  def post_run(self, test, coverage_context=NULL_COVERAGE):
    ...
    self.completion_config.present_result(step_result)
...
class V8SwarmingTest(V8Test):
  def __init__(self, *args, **kwargs):
    ...
    self.task = None
    self.test = None
    self.completion_config = SwarmingCompletionConfig(self.api)
  ...
  def _v8_collect_step(self, task, coverage_context=NULL_COVERAGE, **kwargs):
    ...
    args += self.completion_config.collect_args()
    ...
    with self.api.swarming.on_path():
      return self.api.build.python(
          ...
          infra_step=True,
          step_test_data=self.completion_config.augment_test_data(
              kwargs.pop('step_test_data', None)),
          **kwargs)
  ...
  def pre_run(self, test=None, coverage_context=NULL_COVERAGE, **kwargs):
    ...
    extra_args += self.completion_config.ci_progress_args()
...
class CompletionConfig(object):
  """Handle collection of test completion logs generated by the 'ci' progress
  indicator
  """
  ...
  def augment_test_data(self, existing_data):
    """Add a new test data key for completion logs"""
    completion_data = self.api.v8.test_api.output_text(
        "Done executing test1\nTest failed test 2", name="completion_logs")
    return lambda: completion_data + existing_data()

  def present_result(self, step_result):
    """Show the collected completion logs as separate element in UI"""
    ct = step_result.raw_io.output_texts['completion_logs']
    if ct:
      step_result.presentation.logs['completed tests'] = ct.splitlines()


class DirectCompletionConfig(CompletionConfig):
  """Configure collection for a non-swarming environment"""

  def ci_progress_args(self):
    """Additional arguments for test runner to capture test completion logs in a
    placeholder
    """
    return [
      '--ci-test-completion',
      self.api.raw_io.output_text(name='completion_logs'),
    ]


class SwarmingCompletionConfig(CompletionConfig):
  """Configure collection for a swarming environment"""

  def ci_progress_args(self):
    """Additional arguments for test runner to capture test completion logs in
    the isolated outdir
    """
    return ['--ci-test-completion', '${ISOLATED_OUTDIR}/completed_tests.log']

  def collect_args(self):
    """Additional arguments for collector script to capture concatenated
    completion logs in a placeholder
    """
    return [
