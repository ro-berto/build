...
class PgoApi(recipe_api.RecipeApi):
  ...
  def _profdata_artifact_name(self, sha1):
    ...
    Template:
    chrome-{platform}-{branch number}-{profile_hash}-{timestamp}.profdata
    ...
    * {timestamp} refers to the timestamp of the commit at HEAD.
    ...
    profdata_template = 'chrome-%s-%s-%s.profdata'
    ...
    return profdata_template % (platform, sha1, timestamp)
  ...
  def process_pgo_data(self):
    ...
    with self.m.step.nest('Processing PGO .profraw data'):
      ...
      self.m.profiles.upload(
          self.GS_BUCKET,
          '%s/%s/%s' % (self.GS_BUCKET_PATH, self.branch, new_filename),
          new_filepath,
          args=[
              '-Z',
          ],
