...
_SOURCE_CODE_PATH = os.path.abspath('root' + os.sep +'src')
_BUILD_PATH = os.path.abspath(_SOURCE_CODE_PATH + os.sep
                              + 'out' + os.sep + 'build')
...
class NinjaWrapperTestCase(unittest.TestCase):
  ...
  def testParseDeps(self):
    warning_collector = ninja_wrapper.WarningCollector()
    target_dict = ninja_wrapper.parse_ninja_deps(_DEPS_RETURN, _BUILD_PATH,
                                                 _SOURCE_CODE_PATH,
                                                 warning_collector)
  ...
  def testParseSysDeps(self):
    ...
    target_dict = ninja_wrapper.parse_ninja_deps(
        malformatted_deps, _BUILD_PATH, _SOURCE_CODE_PATH,
        warning_collector)
  ...
  def testParseNinjaGraph(self):
    ...
    graph_dict = graph.get_root_deps(['gen/b.cc'], _BUILD_PATH,
                                     _SOURCE_CODE_PATH)
    expected_dict = {'gen/b.cc': ['../../b.cc']}
    self.assertDictEqual(graph_dict, expected_dict)
    self.assertListEqual(warning_collector.get(), [])

  def testCheckFileTypeAutoGeneratedFile(self):
    file_name = 'gen/a.o'
    file_type = ninja_wrapper.get_file_type(file_name,
                                            _BUILD_PATH, _SOURCE_CODE_PATH)
    self.assertEqual(file_type, ninja_wrapper._AUTO_GENERATED_FILE)

  def testCheckFileTypeSourceCodeFile(self):
    file_name = '../../a.cc'
    file_type = ninja_wrapper.get_file_type(file_name,
                                            _BUILD_PATH, _SOURCE_CODE_PATH)
    self.assertEqual(file_type, ninja_wrapper._SOURCE_FILE)

  def testCheckFileTypeSysDependencies(self):
    file_name = '../../../../a.h'
    file_type = ninja_wrapper.get_file_type(file_name,
                                            _BUILD_PATH, _SOURCE_CODE_PATH)
    self.assertEqual(file_type, ninja_wrapper._SYS_DEPENDENCIES)

  def testCheckRelPath(self):
    file_name_list = ['../../a.cc', '../../b.h']
    expected_rel_path = ['a.cc', 'b.h']
    rel_path = ninja_wrapper.get_relpath_to_source_code(
        file_name_list, _BUILD_PATH, _SOURCE_CODE_PATH)
    self.assertListEqual(rel_path, expected_rel_path)
  ...
  def testGetDetailedInfo(self, _):
    ...
    target_dict = ninja_wrapper.get_detailed_info(
        '', _BUILD_PATH, _SOURCE_CODE_PATH, failed_target_list,
        warning_collector)
  ...
  def testParseArgs(self):
    ...
    self.assertEqual(expected_source_code_path, options.source_code_path)
