...
-s (--source_code_path) argument should also be specified if ninja_info_output
argument is given. Otherwise, ninja wrapper will raise an exception. The ninja
build command should be the last argument after '--' flag. The first argument
of ninja command should be set as ninja's absolute path. Ninja environment
should be set before calling wrapper from recipe:
  with self.m.context(env=ninja_env):
    self.m.python(name, ninja_wrapper.py)

Example:
python ninja_wrapper.py [--ninja_info_output file_name.json --source_code_path
  abs/path] -- /absolute/path/to/ninja -C build/path build_target
...
_AUTO_GENERATED_FILE = 'auto_generated_file'
_SOURCE_FILE = 'source_file'
_SYS_DEPENDENCIES = 'system_dependencies'


def get_file_type(file_name, build_path, source_code_path):
  # File name is relative path to build_path.
  file_path = os.path.abspath(os.path.join(build_path, file_name))
  build_path_prefix = os.path.commonprefix([file_path, build_path])
  # We assume that source code path is not a subdirectory of build path.
  # If a file resides in the output path, it must be auto-generated.
  if build_path_prefix == build_path:
    return _AUTO_GENERATED_FILE
  # If a file resides in source code path and not in output path, it must be
  # source code.
  source_code_prefix = os.path.commonprefix([file_path, source_code_path])
  if source_code_prefix == source_code_path:
    return _SOURCE_FILE
  return _SYS_DEPENDENCIES


def get_relpath_to_source_code(file_name_list, build_path,
                               source_code_path):
  result = []
  for file_name in file_name_list:
    file_path = os.path.abspath(os.path.join(build_path, file_name))
    rel_path = os.path.relpath(file_path, source_code_path)
    rel_path.replace('\\', '/')
...
class Graph(object):
  ...
  def get_root_deps(self, node_names, build_path, source_code_path):
      ...
      while edge_list:
        ...
        for input_node in edge.normal_input_nodes:
          if not input_node.input_edge:
            file_type = get_file_type(input_node.node_name,
                                      build_path, source_code_path)
            if file_type is _SOURCE_FILE:
...
def parse_ninja_deps(ninja_deps_output, build_path,
                     source_code_path, warning_collector):
  ...
  Args:
    ...
    source_code_path: a string representing absolute path of source code.
    ...
    if match:
      ...
      for dep in lines[index:index + deps_num]:
        ...
        if not dep:
          ...
          continue
        file_type = get_file_type(dep, build_path,
                                  source_code_path)
        if file_type is _AUTO_GENERATED_FILE:
          deps_info.auto_generated_deps.append(dep)
        elif file_type is _SOURCE_FILE:
...
def get_detailed_info(ninja_path, build_path, source_code_path,
  ...
  if failed_nodes:
    ...
    deps_dict = parse_ninja_deps(ninja_deps_output, build_path,
                                 source_code_path, warning_collector)
    auto_generated_deps = []
    ...
    for target in failed_target_list:
      ...
      target['dependencies'] = get_relpath_to_source_code(
          list(set(target['dependencies'])), build_path, source_code_path)
...
def parse_args(args):
  ...
  parser.add_argument('--ninja_info_output',
                      help=('Optional. Save result in file.'))
  parser.add_argument('--source_code_path',
                      help=('Optional. Absolute path to source code.'
                            'This argument is required if ninja_info_output '
                            'argument is given.'))
...
def main():
  ...
  # If source_code_path argument isn't given, calls ninja directly.
  ...
  # If source_code_path argument isn't given or is empty, exception
  # will be raise.
  assert source_code_path, 'source_code_path argument is required.'
  ...
  # Explicitly close stdout pipe without waiting for gc to take care of it.
  popen.stdout.close()
  return_code = popen.wait()
  if return_code:
    prev_arg = None
    build_path = os.cwd()
    for arg in ninja_cmd:
      if prev_arg == '-C':
        build_path = arg
        break
      prev_arg = arg
