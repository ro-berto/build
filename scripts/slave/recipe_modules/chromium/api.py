...
class ChromiumApi(recipe_api.RecipeApi):
  ...
  def runtest(self, test, args=None, xvfb=False, name=None, annotate=None,
              ...
              chartjson_file=False, disable_src_side_runtest_py=False,
  ...
  def run_mb(self, mastername, buildername, use_goma=True, mb_path=None,
    ...
    Args:
      gn_args_presenter: The callback used to present the GN args. It must be an
        object callable with two arguments:
          1. The step result.
          2. An OrderedDict containing the GN args for the build, mapping the
            argument name to its value.
        By default, chromium.default_gn_args_presenter will be used. To change
        the text limit or force it to present to the logs or text, use
        functools.partial. e.g.
          functools.partial(
             self.m.chromium.default_gn_args_presenter, location='logs')

    Returns:
      An OrderedDict containing the GN args for the build, mapping the argument
      name to its value.
    """
    ...
    return self._gn_args(
        build_dir, gn_args_presenter or self.default_gn_args_presenter)

  @staticmethod
  def default_gn_args_presenter(result, gn_args, location=None, text_limit=15):
    ...
    Args:
      ...
      gn_args: The GN args to present.
      location: Controls where the GN args for the build should be presented. By
        default or if None, the args will be in step_text if the count is less
        than text_limit or the logs otherwise. To force the presentation to the
        step_text or logs, use 'text' or 'logs', respectively.
     text_limit: The maximum number of GN args to display in the step_text when
        using the default behavior for displaying GN args.
    """
    ...
    # presentation
    NON_LOCAL = ['goma_dir']
    sorted_lines = ['%s = %s' % arg for arg in
                    sorted(gn_args.iteritems(),
                           key=lambda item: item[0] in NON_LOCAL)]

    if location is None:
      if len(gn_args) > text_limit:
        result.presentation.text = (
            '<br/>Count of GN args (%d) exceeds limit (%d),'
            ' presented in logs instead') % (len(gn_args), text_limit)
        location = 'logs'

    if location == 'logs':
      result.presentation.logs['gn_args'] = sorted_lines
    else:
      result.presentation.step_text = '<br/>'.join([''] + sorted_lines)


  def _gn_args(self, build_dir, presenter):
    """Get the GN args for the build.

    Args:
      build_dir: The output directory generated by mb. The args.gn file will be
        extracted from this location.
    ...
    if isinstance(build_dir, str):
      build_dir = self.m.path['checkout'].join(
          *build_dir.lstrip('/').split('/'))

    gn_args_path = build_dir.join('args.gn')
    fake_gn_args = (
        'goma_dir = "/b/build/slave/cache/goma_client"\n'
        'target_cpu = "x86"\n'
        'use_goma = true\n')
    gn_args_content = self.m.file.read_text(
        'GN args', gn_args_path, fake_gn_args)

    gn_args = collections.OrderedDict()
    for arg in gn_args_content.splitlines():
      name, value = arg.split('=', 1)
      gn_args[name.strip()] = value.strip()

    presenter(self.m.step.active_result, gn_args)
