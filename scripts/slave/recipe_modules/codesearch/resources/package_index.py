...
class ProtoTarget():
  ...
  def __init__(self, gn_target, root_dir, out_dir):
    ...
    self.proto_in_dirs = [
        os.path.normpath(
            os.path.join(self.root_dir, self.out_dir, self.args[i + 1]))
        for i in range(len(self.args) - 1)
        if self.args[i] == '--proto-in-dir'
    ]

    if not self.proto_in_dirs:
      self.proto_in_dirs = [os.path.join(self.root_dir, out_dir)]
  ...
  def GetUnit(self, corpus, filehashes, build_config=None):
    ...
    for f in sorted(self.GetFiles()):
      if f not in filehashes:
        print('missing file %s in filehashes' % f)
        continue
      required_input = unit_proto.required_input.add()
      required_input.v_name.corpus = corpus
      required_input.v_name.path = os.path.relpath(
          ConvertPathToForwardSlashes(os.path.normpath(f)), self.root_dir)
      required_input.info.path = os.path.relpath(
          ConvertPathToForwardSlashes(f),
  ...
  def _FindAllUsedFiles(self):
    ...
    paths = [
        os.path.join(self.root_dir, 'src', source[2:])
        for source in self.sources
    ]
    all_files = set()
    while len(paths):
      ...
      if path in all_files:
        ...
        continue
      all_files.add(path)
      for imp in FindImports(PROTO_IMPORT_RE, path, self.proto_in_dirs):
        paths.append(imp)
    return all_files
...
class IndexPack(object):
  ...
  def _GenerateUnitFiles(self):
    ...
    for target in self.proto_targets:
      self._AddUnitFile(
          target.GetUnit(self.corpus, self.filehashes, self.build_config))
