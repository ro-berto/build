...
class ProtoTargetTest(unittest.TestCase):
  ...
  def setUp(self):
    self.root = 'C:\\' if sys.platform == 'win32' else '/foo'
  ...
  def testNoProtoInDir(self, findImportsMock):
    findImportsMock.side_effect = self.ImportMockGen(
        ...
        [os.path.join(self.root, 'src', 'out', 'debug')], {
            os.path.join(self.root, 'src', 'orig.proto'):
                set([
                    os.path.join(self.root, 'src', 'out', 'debug', 'baz.proto')
                ]),
            os.path.join(self.root, 'src', 'out', 'debug', 'baz.proto'):
                set(),
        })
  ...
  def testProtoInDir(self, findImportsMock):
    abspath = 'C:\\' if sys.platform == 'win32' else '/abspath'
    findImportsMock.side_effect = self.ImportMockGen(
        package_index.PROTO_IMPORT_RE,
        [os.path.join(self.root, 'src'), abspath], {
            os.path.join(self.root, 'src', 'orig.proto'):
                set([os.path.join(abspath, 'baz.proto')]),
            os.path.join(self.root, 'src', 'orig2.proto'):
                set(),
            os.path.join(abspath, 'baz.proto'):
                set(),
        })
    ...
    filehashes = {
        os.path.join(self.root, 'src', 'orig.proto'): '1',
        os.path.join(self.root, 'src', 'orig2.proto'): '2',
        os.path.join(abspath, 'baz.proto'): '3',
    }
  ...
  def testRecusiveImport(self, findImportsMock):
    findImportsMock.side_effect = self.ImportMockGen(
        package_index.PROTO_IMPORT_RE, [os.path.join(self.root, 'src')], {
            os.path.join(self.root, 'src', 'orig.proto'):
                set([os.path.join(self.root, 'src', 'import.proto')]),
            os.path.join(self.root, 'src', 'import.proto'):
                set([os.path.join(self.root, 'src', 'import2.proto')]),
            os.path.join(self.root, 'src', 'import2.proto'):
                set(),
        })
    ...
    filehashes = {
        os.path.join(self.root, 'src', 'orig.proto'): '1',
        os.path.join(self.root, 'src', 'import.proto'): '2',
        os.path.join(self.root, 'src', 'import2.proto'): '3',
    }
    unit_proto = pt.GetUnit('corpusname', filehashes, 'build_config')
    self.assertIsNotNone(unit_proto)
