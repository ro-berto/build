...
class ChromeOSCommands(commands.FactoryCommands):
  ...
  def AddChromeOSImageStep(self, clobber=False, mode=None, options=None,
    ...
    if options and options.get('lasercats'):
      ...
      cmd = self._enter_chroot + [
            './build_image '
            '--replace --board ' + self.BoardName(options) +
            fast
      ]
    else:
  ...
  def AddChromeOSFactoryInstallStep(self, clobber=False, mode=None, options=None,
    ...
    cmd = self._enter_chroot + [
      ...
      ' --force_copy',
	    ...
	    '--to=../build/images/%s/foo/chromiumos_factory_install_image.bin' % board,
		]
    self._factory.addStep(shell.ShellCommand,
                          name='factory image',
  ...
  def AddChromeOSTestSteps(self, tests, clobber=False, mode=None, options=None,
    ...
    if 'platform' in tests:
      ...
      cmd = self._enter_chroot + [
          './cros_run_unit_tests --board ' + self.BoardName(options)]
      self._factory.addStep(shell.ShellCommand,
                            name='run tests',
  ...
  def AddChromeOSPreFlightStep(self, command, clobber=False, mode=None,
    ...
    if command == 'clean':
      ...
      cmd = self._enter_chroot + [
            './cros_mark_all_as_stable --board %s' % self.BoardName(options)]
