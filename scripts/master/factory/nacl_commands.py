...
class NativeClientCommands(commands.FactoryCommands):
  ...
  def __init__(self, factory=None, identifier=None, target=None,
    ...
    self._build_compile_name = 'scons compile'
    if 'dbg' in target:
      configuration = 'Debug'
    else:
  ...
  def AddArchiveCoverage(self, coverage_dir):
    ...
    url = ('http://gsdview.appspot.com/nativeclient-coverage/'
           'revs/%(got_revision)s/' + coverage_dir + '/html/index.html')
    text = 'view'

    src = 'native_client/scons-out/%s/coverage' % coverage_dir
    dst_path = 'nativeclient-coverage/revs/%(got_revision)s/' + coverage_dir
    dst = 'gs://' + dst_path
    ...
    if self._target_platform.startswith('win'):
      env['GSUTIL'] = r'c:\b\scripts\slave\gsutil.bat'
      env['HOME'] = r'c:\Users\chrome-bot'

    cmd = [self._python, '/b/scripts/slave/gsutil_cp_dir.py',
           '--message', url, src, dst]
  ...
  def AddArchiveBuild(self, src, dst_base, dst, mode='dev', show_url=True):
    ...
    cmd = [self._python, '/b/scripts/command_wrapper/bin/command_wrapper.py',
           '--', self._python, self._gsutil_tool, 'cp', src, full_dst]
    cmd2 = [self._python, '/b/scripts/command_wrapper/bin/command_wrapper.py',
  ...
  def AddModularBuildStep(self, mode, clobber=False,
                          options=None, timeout=1200):
    if clobber:
      self._factory.addStep(shell.ShellCommand,
                            description='clobber_modular_build',
                            timeout=timeout,
                            haltOnFailure=True,
    ...
    self._factory.addStep(shell.ShellCommand,
                          ...
                          haltOnFailure=True,
                          workdir='build/native_client/tools/modular-build',
                          command='python build.py -b')

  def AddSpecStep(self, name, command, timeout=1200,
                  haltOnFailure=True):
    self._factory.addStep(shell.ShellCommand,
                          description=name,
                          timeout=timeout,
                          haltOnFailure=haltOnFailure,
                          workdir='build/native_client/tests/spec2k',
                          command=command)
