...
class SkiaFactory(gclient_factory.GClientFactory):
  ...
  def Build(self, clobber=None):
    ...
    # Rerun GM, storing actual results on local disk to aid in rebaselining.
    #
    # TODO(epoger): This is a hacky fix to
    # http://code.google.com/p/skia/issues/detail?id=386 ('make buildbots write
    # out RunGM image results to a browsable directory'). Ultimately, we need
    # the actual GM results to be stored somewhere OTHER than on the buildbot
    # slave itself.
    # Elliot promises to remove this in favor of a long-term fix before
    # 1 Dec 2011.
    gm_output_dir = self.TargetPathJoin(
        self._gm_actual_dir, 'r%%(%s:-)s' % 'revision')
    if self._target_platform == TARGET_PLATFORM_WIN32:
      command = WithProperties('mkdir %s && %s -w %s' % (
          gm_output_dir, path_to_gm, gm_output_dir))
    else:
      command = WithProperties('mkdir -p %s && %s -w %s && chmod -R a+rX %s' % (
          gm_output_dir, path_to_gm, gm_output_dir, gm_output_dir))
    self._skia_cmd_obj.AddRun(run_command=command, description='StoreGMResults')
