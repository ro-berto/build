...
class LibjingleFactory(gclient_factory.GClientFactory):

  CUSTOM_VARS_ROOT_DIR = ('root_dir', 'src')
  # Can't use the same Valgrind constant as in chromium_factory.py, since
  # Libjingle uses another path (use_relative_paths=True in DEPS).
  ...
  def __init__(self, build_dir, target_platform):
    ...
    svn_url = config.Master.libjingle_url + '/trunk'
    solutions = []
  ...
  def LibjingleFactory(self, target='Debug', clobber=False, tests=None,
    ...
    valgrind_script_path = cmds.PathJoin('src', 'tools', 'valgrind-libjingle')
